...
const Footer = () => {
  const { navLogo, contactInfo } = useData();
    ...
    <footer className="bg-[#010409] border-t border-white/10 pt-16 pb-8 relative z-10 mt-auto">
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-12 mb-12">
        ...
        <div><h4 className="text-white font-bold mb-6">Quick Links</h4><ul className="space-y-3 text-sm text-gray-400">
          ...
          <li><Link href="/#about" className="hover:text-[#0ea5e9] flex items-center gap-2">› About</Link></li>
          <li><Link href="/#contact" className="hover:text-[#0ea5e9] flex items-center gap-2">› Contact</Link></li>
        </ul></div>
...
const Navbar = () => {
  const { navLogo, navSizes, isMobileMenuOpen, setIsMobileMenuOpen, currentUser } = useData();
  return (
    <>
      <style jsx global>{`
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#020617]/95 backdrop-blur-md border-b border-white/10 shadow-lg h-32 flex items-center transition-all">
        <div className="max-w-7xl mx-auto px-6 w-full flex items-center justify-between h-full">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold text-gray-300 tracking-wide">
            ...
            <Link href="/#research" className="hover:text-[#0ea5e9] transition-colors">Research</Link>
            <Link href="/#careers" className="hover:text-[#0ea5e9] transition-colors">Careers</Link>
            <Link href="/#about" className="hover:text-[#0ea5e9] transition-colors">About</Link>
            <Link href="/#contact" className="hover:text-[#0ea5e9] transition-colors">Contact</Link>
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[#0f172a] border-t border-white/10 absolute w-full left-0 top-full px-6 py-8 flex flex-col gap-6 shadow-2xl h-screen">
            ...
            <Link href="/#about" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-white hover:text-[#0ea5e9]">About</Link>
            <Link href="/#contact" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-white hover:text-[#0ea5e9]">Contact</Link>
            <Link href="/entry-manage" onClick={() => setIsMobileMenuOpen(false)}><button className="w-full bg-[#0ea5e9] text-black font-bold py-4 rounded-xl mt-4">{currentUser ? "Dashboard" : "Sign In"}</button></Link>
          </div>
        )}
      </nav>
    </>
