...
export default function Home() {
  ...
  return (
    <main className="flex-col select-none relative z-0">
      ...
      <AnimatePresence>
      {selectedJob && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm">
      ...
      {/* --- GALLERY MODAL (FIXED: z-100 & pt-40) --- */}
      <AnimatePresence>
        {showGallery && (
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className="fixed inset-0 z-[100] bg-[#020617] overflow-y-auto" // Solid BG + High Z-Index
          >
             <button className="fixed top-6 right-6 z-[110] text-white/70 hover:text-white bg-white/10 p-2 rounded-full transition-colors" onClick={() => setShowGallery(false)}><X size={32}/></button>
             
             {/* CONTENT CONTAINER WITH PADDING-TOP FIX */}
